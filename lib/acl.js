...
*/
Acl.prototype.middleware = function (role, {
  resource,
  actions, 
}) {
  contract(arguments)
  ...
  return async function (req, res, next) {
    ...
    _debug ? acl.logger.debug('Requesting ' + _actions + ' on ' + _resource + ' by user with role ' + _role) : null;

    acl.isAllowed(_role, _resource, _actions, function (err, allowed) {
      if (err) {
        next(new Error('Error checking permissions to access resource'));
      } else if (allowed === false) {
        if (_debug) {
          acl.logger.debug('Not allowed ' + _actions + ' on ' + _resource + ' by user with role ' + _role);
          acl.allowedPermissions(_role, _resource, function (err, obj) {
        ...
        if (_debug) {
          acl.logger.debug('Allowed ' + _actions + ' on ' + _resource + ' by user with role ' + _role);
        }
