 ...
 * @param {Object|null}
 */
Acl.prototype.middleware = function (user, {
  resource,
  actions, 
} = {}) {
  contract(arguments)
    .params('string|number|function')
    .params('string|number|function', 'object')
    .end();
  ...
  return async function (req, res, next) {
    let _user = user,
      _actions = actions,
      _resource = resource,
      _debug = acl.logger && acl.options.hasOwnProperty('debug') && acl.options.debug === true;
    ...
    _debug ? acl.logger.debug('Requesting ' + _actions + ' on ' + _resource + ' by user with id ' + _user) : null;

    acl.isAllowed(_user, _resource, _actions, function (err, allowed) {
      if (err) {
        next(new Error('Error checking permissions to access resource'));
      } else if (allowed === false) {
        if (_debug) {
          acl.logger.debug('Not allowed ' + _actions + ' on ' + _resource + ' by user with id ' + _user);
          acl.allowedPermissions(_user, _resource, function (err, obj) {
        ...
        if (_debug) {
          acl.logger.debug('Allowed ' + _actions + ' on ' + _resource + ' by user with id ' + _user);
        }
